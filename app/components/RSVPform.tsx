...
import { addGuest, FormState } from '../actions';
    ...
    <div className="w-full max-w-md p-8 bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/20">
      {formState.success ? (
        <div className="text-center">
          <h2 className="text-3xl font-bold text-white mb-3">{formState.success}</h2>
          <p className="text-gray-200">You're on the list. See you there.</p>
        </div>
      ) : (
        <>
          <h2 className="text-3xl font-bold text-center text-white mb-6">Join the Party</h2>
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              ...
              />
              {formState.error && (
                <p className="text-red-400 text-sm mt-2 text-left">{formState.error}</p>
              )}
            </div>
            <button
              type="submit"
